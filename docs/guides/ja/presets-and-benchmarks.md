...
> **注意:** すべてのプリセットは `search_dim` を `None` に設定するようになりました。エンジンは実行時に evaluator の `search_dim()` メソッドを照会して、探索空間の次元を決定します。

すべてのプリセットはデフォルトで `DefaultSearch` を使用します。`StratifiedSearch` は、
座標ステップ / 方向探索 / Halton 探索を組み合わせた戦略を必要とするユーザー向けに、
`PolicyBundle::Search` を通じたドロップイン置換として利用可能です（[アーキテクチャ](architecture.md#stratifiedsearch)を参照）。

異方的メッシュジオメトリは、`EngineConfig.mesh_base_steps` を次元ごとのステップサイズを持つ
`Some(vec![...])` に設定することで、任意のプリセットで有効にできます。
未設定の場合、デフォルトの等方的メッシュが使用されます。
